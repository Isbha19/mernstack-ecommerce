...
//Fetch orders stats
export const getOrdersStatisticsAction = createAsyncThunk(
  "orders/stats",
  async (payload, { rejectWithValue, getState, dispatch }) => {
    try {
      //token
...
export const updateOrderAction = createAsyncThunk(
  "order/update-order",
  async ({ status, id }, { rejectWithValue, getState, dispatch }) => {
    try {
      //token
      const token = getState().users?.userAuth?.userInfo?.token;
      const config = {
...
const ordersSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    //fetch order stats
    builder.addCase(getOrdersStatisticsAction.pending, (state) => {
      state.loading = true;
    });
    builder.addCase(getOrdersStatisticsAction.fulfilled, (state, action) => {
      state.loading = false;
      state.stats = action.payload;
    });
    builder.addCase(getOrdersStatisticsAction.rejected, (state, action) => {
      state.loading = false;
      state.stats = null;
